...
tb_sdg<-read.csv("Test for IndicatorDB (Antworten).csv")
...
tb_sdg2 <- tb_sdg %>% # filling countries if only global or continent
  mutate(country_init = `Which.countries.are.covered..1`,
         ...
         country_init = ifelse(`Geographic.coverage` == "Continental",
                               paste(geo_tags$country[geo_tags$continent == "Asia"], 
                                     collapse = ","),
                               country_init)) %>% # Separate contry
  separate(`Which.countries.are.covered..1`, country_vars, sep = ",", remove = FALSE) %>%
